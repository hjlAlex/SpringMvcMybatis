...
import com.wabao.util.PackageUtil;
...
public class TableSchemaUpdate {
	...
	public TableSchemaUpdate(AutoCreateTableDao autoCreateTableDao,
		...
		this.schema = schema;
		this.entityClasses = PackageUtil.getClasses(this.entityPath);
	...
	public void run() {
		...
		for (Class<?> entity : this.entityClasses) {
			...
			if (exist == 0) {// 不存在,需要新增该表
				...
				List<String> existColumnNames = PackageUtil
						.getPropertyValueList(tableExistColumnList,
		...
		log.info("执行扫描和更新表结构结束..." + "耗时:" + (e - s) + "ms");
	}
		...
		for (SysMysqlColumns sysColumn : tableColumnList) {
			...
			if (createTableParam != null) {
				...
				if ("PRI".equals(sysColumn.getColumn_key().toUpperCase())
					...
					dropKeyFieldList.add(createTableParam);// 该字段虽然属于删除唯一约束字段,但是后面的逻辑还要执行(参考如下业务场景:我需要把这个字段的主键删了,同时字段长度也变更)
				}
